...
Date=$(date|cut -f2,3,6 -d' '|sed 's/ //g')
...
do
		...
		pxtrt -t /ufrc/soltis/cactus/Dimension/Community_Opentree/Stephen_Brown_tree/vas_opentree_9.1.tre -f ${Site}_justids.ottids >${Site}_ottid_${Date}.tre 2>${Site}_miss_match_back_${Date}.txt
		
		sed -i 's/ not in tree//g' ${Site}_miss_match_back_${Date}.txt
		...
		pxtrt -t /ufrc/soltis/cactus/Dimension/Community_Opentree/Stephen_Brown_tree/ALLOTB_ottid.tre -f ${Site}_justids.ottids >${Site}_BR_ottid_${Date}.tre 2>${Site}_BR_miss_match_back_${Date}.txt
		
		sed -i 's/ not in tree//g' ${Site}_BR_miss_match_back_${Date}.txt
